...
let startIndex = content.indexOf('export const airports = ');
let arrayString = content.substring(startIndex + 'export const airports = '.length);
arrayString = arrayString.trim();
if (arrayString.endsWith(';')) {
  arrayString = arrayString.substring(0, arrayString.length - 1);
}

let airports = JSON.parse(arrayString);
...
mappings.forEach(({ fullName, url }) => {
  ...
  for (let airport of airports) {
    if (
      fullName.includes(airport.name) || 
      airport.name.includes(fullName.split(' ')[0]) || 
      (airport.name === 'Firefly' && fullName.includes('飞为')) || 
      (airport.name === 'FlyV' && fullName.includes('飞V')) ||
      (airport.name === '无忧链接' && fullName === '无忧') ||
      (airport.name === '跨界云' && fullName === '跨界')
    ) {
...
const newContent = 'export const airports = ' + JSON.stringify(airports, null, 2) + ';\n';
